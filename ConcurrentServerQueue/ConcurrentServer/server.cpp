...
static HANDLE QueueDispatchHandle;

static list<contact> Contacts;
static queue<contact> WaitingClients;
...
DWORD WINAPI QueueDispatchThread(LPVOID Param){	
	DWORD Result = 0;
	printf("QueueDispatch");

	talkers_command* Cmd = (talkers_command*)Param;

	while (*Cmd != TC_EXIT){

		if ((Contacts.size() < CLIENT_QUEUE_SIZE) &&
			(WaitingClients.size() > 0))
		{
			EnterCriticalSection(&OurCriticalSection);

			Contacts.push_front(WaitingClients.front());
			Contacts.front().WaitingState = WAIT_TS_CONTACT;
			WaitingClients.pop();
			SetEvent(OurEvent);
	...
	printf("Exiting QueueDispatch Thread\n");
	return(Result);
}
...
int main(int argc, char** argv){
	...
	QueueDispatchHandle = CreateThread(0, 0, QueueDispatchThread, (LPVOID)&cmd, 0, 0);
