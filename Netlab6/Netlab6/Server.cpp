...
int main(int argc, char** argv){
	...
	if (ServerHandle != INVALID_HANDLE_VALUE){
		...
		GetFileSizeEx(ServerHandle, &FileSize);
		void* Contents = VirtualAlloc(0, FileSize.QuadPart, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
		DWORD BytesRead;
		ReadFile(ServerHandle, (char*)Contents, FileSize.QuadPart, &BytesRead, 0);
		printf("%s\n", Contents);
		...
		for (int i = 0; i < 100; i++){
			...
			char* InputBuffer = (char*)calloc(256, sizeof(char));
			DWORD BytesRead;
			CallNamedPipeA(NamedPipeName, InputBuffer, 256, OutputBuffer, strlen(OutputBuffer) + 1, &BytesRead, 0);
			printf("%s\n", InputBuffer);
