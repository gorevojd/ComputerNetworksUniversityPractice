...
static CRITICAL_SECTION WarnCriticalSection;
...
static map<std::string, warned_note> WarnedNotes;
		...
		if (*Command != TC_EXIT && NumberOfTries > 0){
			...
			while (Tmp > 0 && ClientIsConnected == false){
				...
				if (Cont.Sock == INVALID_SOCKET){
					...
					char* IpStr = inet_ntoa(Cont.SockAddr.sin_addr);

					EnterCriticalSection(&OurCriticalSection);
					map<std::string , warned_note>::iterator TempIt = WarnedNotes.find(IpStr);
					bool IsElementInMap = (TempIt != WarnedNotes.end());
					if (IsElementInMap == true){
						Cont.IsBanned = WarnedNotes[IpStr].IsBanned;
						Cont.IsWarned = WarnedNotes[IpStr].IsWarned;
					}
					else{
						WarnedNotes.insert(std::pair<char*, warned_note>(IpStr, warned_note()));
						EnterCriticalSection(&WarnCriticalSection);
					}
...
#define BadWordsCount 4
	static char* BadWords[BadWordsCount] = { "Lox", "lox", "fuck", "Fuck" };
	...
	while ((ContinueLoop == true) && (Cont->IsTimerEnded == false) && Cont->GlobalRunning){
		...
		if (Cont->IsBanned != true){
			...
			if (ReceivedBytesCount != -1 && Cont->GlobalRunning){
				...
				int LastWordBegin = 0;
				for (int StrIndex = 0; StrIndex < strlen(InputBuffer); StrIndex++){
					char TempWord[64];
					int WordCharIndex = 0;
					if (InputBuffer[StrIndex] == ' '){
						for (int z = LastWordBegin; z < StrIndex; z++){
							TempWord[WordCharIndex++] = InputBuffer[z];
						}
						TempWord[WordCharIndex] = 0;

						LastWordBegin = StrIndex + 1;

						for (int BadWordIndex = 0; BadWordIndex < BadWordsCount; BadWordIndex++){
							if (strcmp(TempWord, BadWords[BadWordIndex]) == 0){
								if (Cont->IsWarned == true){
									Cont->IsBanned = true;
									printf("You have been banned!!!\n");
								}
								else{
									Cont->IsWarned = true;
									printf("Do not say bad words, my friend!!!\n");
								}
								break;
							}
						}
				...
				if (Cont->IsWarned == true){
					char AnswerStr[] = "Your query contains bad words\n";
					if (Cont->IsBanned == true){
						char AnswerStr2[] = "You have been banned!!!\n";
						int SentBytesCountBanned = send(Cont->Sock, AnswerStr2, strlen(AnswerStr2) + 1, NULL);
					}
					else{
						int SentBytesCountWarned = send(Cont->Sock, AnswerStr, strlen(AnswerStr) + 1, NULL);
					}
				}
				else{
		...
		for (int i = 0; i < Contacts.size(); i++){
			...
			if (it != Contacts.end()){
				...
				if (Ts == WORK_TS_FINISH || Ts == WORK_TS_ABORT || Ts == WORK_TS_TIMEOUT){
					...
					char* IpStr = inet_ntoa(it->SockAddr.sin_addr);
					auto TempIt = WarnedNotes.find(IpStr);
					bool IsElementInMap = (TempIt != WarnedNotes.end());
					printf("Warns Size: %i Str: %s\n", WarnedNotes.size(), WarnedNotes.begin()->first);

					if (IsElementInMap == true){
						WarnedNotes[IpStr].IsBanned = it->IsBanned;
						WarnedNotes[IpStr].IsWarned = it->IsWarned;
						//TempIt->second.IsBanned = it->IsBanned;
						//TempIt->second.IsWarned = it->IsWarned;
						printf("IsBanned: %i\n", it->IsBanned);
						printf("IsWarned: %i\n", it->IsWarned);
					}
...
int main(int argc, char** argv){
	...
	InitializeCriticalSection(&WarnCriticalSection);
	...
	DeleteCriticalSection(&WarnCriticalSection);
